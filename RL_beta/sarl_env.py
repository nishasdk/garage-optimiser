...
try:
    ...
    raise DependencyNotInstalled("box2d is not installed, run `pip install 'gym[box2d]'`") from e

if TYPE_CHECKING:
    import pygame


##TODO add a separate function for checking termination requirements? or any wrappers 

class LunarLander(gym.Env, EzPickle):
    """
    ### Description
    Environment simulates a 20-year timespan in which a parking garage operates.
    The design has the option to expand however many floors it deems fit

    The actions are always discrete.
    The capacity of the garage has a minimum of 2 floors and a maximum of 9 floors
    The goal is to maximise the NPV over 20 years under stochastic demand. It is 
    possible to achieve a negative NPV.
    The game terminates after 20 years

    ### Action Space
    There are six discrete actions available: do nothing, and expand by one,
    two, three, four or five floors.

    ### Observation Space
    The state is an 2-dimensional vector: the coordinates of the lander in `x` & `y`, its linear
