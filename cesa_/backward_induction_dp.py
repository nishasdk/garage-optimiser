...
class TransitionData(dict):
    ...
    def __setitem__(self, key, value):
        ...
        if (
            (m not in self.states)
            or (n not in self.states)
            or (t not in range(self.number_of_stages))
            or (x not in self.decisions)
        ):
...
class StateData(dict):
    ...
    def __repr__(self):
        ...
        for n, v in sorted(self.items()):
            entries.append(" (state: {0}): {1}".format(n, v))
        string = "{" + "\n".join(entries).strip() + "}"
        return string
...
class StageStateData(dict):
    ...
    def __setitem__(self, key, value):
        ...
        if (n not in self.states) or (t not in range(self.number_of_stages)):
...
class StochasticDP(object):
    def __init__(self, number_of_stages, states, decisions, minimize=True):
        ...
        self.probability = TransitionData(
            self.number_of_stages, self.states, self.decisions
        )

        # Initialize contributions
        self.contribution = TransitionData(
            self.number_of_stages, self.states, self.decisions
        )
    ...
    def add_transition(
        self, stage, from_state, decision, to_state, probability, contribution
    ):
        if (
            self.probability[to_state, from_state, stage, decision]
            or self.contribution[to_state, from_state, stage, decision]
        ):
            warn(
                "The transition from state {0} to state {1} in stage {2} "
                "under decision {3} is already defined. You are overwriting "
                "this transition data".format(from_state, to_state, stage, decision)
            )
    ...
    def add_boundary(self, state, value):
        if self.boundary[state]:
            warn(
                "The boundary condition at state {0} is already defined. "
                "You are overwriting this boundary condition".format(state)
            )
    ...
    def solve(self):
                ...
                for x in self.decisions:
                    ...
                    if isclose(sum_probability, 1):
                        ...
                        raise ValueError(
                            ...
                            "from state {0} at stage {1} under decision {2}".format(
                                n, t, x
                            )
                        )
        ...
        for n in self.states:
            if self.boundary[n] is None:
                raise ValueError(
                    "Dynamic program is not well-defined. "
                    "Check the boundary condition for state {0}".format(n)
                )
            else:
